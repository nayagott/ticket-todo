...
describe('DetailModal — 가시성', () => {
  it('ticket=null → 렌더링되지 않음', () => {
    render(<DetailModal ticket={null} onClose={jest.fn()} onUpdated={jest.fn()} onDeleted={jest.fn()} />);
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
  });

  it('ticket 있음 → role="dialog", aria-modal="true"', () => {
    render(<DetailModal ticket={baseTicket} onClose={jest.fn()} onUpdated={jest.fn()} onDeleted={jest.fn()} />);
...
describe('DetailModal — 인라인 편집', () => {
  ...
  it('title 수정 후 blur → PATCH 호출 + onUpdated', async () => {
    const updated: TicketDto = { ...baseTicket, title: '수정된 제목' };
    server.use(
      http.patch('/api/tickets/detail-1', () => HttpResponse.json(updated)),
    );
    const onUpdated = jest.fn();
    render(<DetailModal ticket={baseTicket} onClose={jest.fn()} onUpdated={onUpdated} onDeleted={jest.fn()} />);
    await userEvent.click(screen.getByText('상세 보기 티켓'));
    const input = screen.getByDisplayValue('상세 보기 티켓');
    await userEvent.clear(input);
    await userEvent.type(input, '수정된 제목');
    await userEvent.tab(); // blur 발생
    await waitFor(() => expect(onUpdated).toHaveBeenCalledWith(expect.objectContaining({ title: '수정된 제목' })));
  });

  it('title 비운 뒤 blur → API 호출 없음, 에러 표시 (NFR-016)', async () => {
    const patchSpy = jest.fn();
    server.use(http.patch('/api/tickets/detail-1', () => { patchSpy(); return HttpResponse.json({}); }));
    render(<DetailModal ticket={baseTicket} onClose={jest.fn()} onUpdated={jest.fn()} onDeleted={jest.fn()} />);
    await userEvent.click(screen.getByText('상세 보기 티켓'));
    const input = screen.getByDisplayValue('상세 보기 티켓');
    await userEvent.clear(input);
...
describe('DetailModal — 삭제 흐름 (FR-005, NFR-017)', () => {
  ...
  it('ConfirmDialog 확인 → DELETE 호출 + onDeleted', async () => {
    server.use(http.delete('/api/tickets/detail-1', () => new HttpResponse(null, { status: 204 })));
    const onDeleted = jest.fn();
    render(<DetailModal ticket={baseTicket} onClose={jest.fn()} onUpdated={jest.fn()} onDeleted={onDeleted} />);
    // DetailModal 삭제 버튼 → ConfirmDialog 열기
    await userEvent.click(screen.getByRole('button', { name: '삭제' }));
    // ConfirmDialog 내 확인(삭제) 버튼 — alertdialog 내부에서 선택
    const { within } = await import('@testing-library/react');
    await userEvent.click(within(screen.getByRole('alertdialog')).getByRole('button', { name: '삭제' }));
    await waitFor(() => expect(onDeleted).toHaveBeenCalledWith('detail-1'));
  });
});
...
describe('DetailModal — 닫기', () => {
  it('닫기 버튼 클릭 → onClose 호출', async () => {
    ...
    render(<DetailModal ticket={baseTicket} onClose={onClose} onUpdated={jest.fn()} onDeleted={jest.fn()} />);
